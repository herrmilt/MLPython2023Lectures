...
from .utils import Detection, resize_image, load_labels, nms


class SSDAnnette(Model):
    ...
    def _get_output_parser(self, net, image_blob_name, bboxes='bboxes', labels='labels', scores='scores'):
        ...
        try:
            parser = MultipleOutputParser(net.outputs, "boxes", "scores", "scores")
    ...
    def preprocess(self, inputs):
        ...
        resized_image = resize_image(image, (self.w, self.h), False)
...
class SingleOutputParser:
    ...
    def __call__(self, outputs):
        return [Detection(xmin, ymin, xmax, ymax, score, label)
                for _, label, score, xmin, ymin, xmax, ymax in outputs[self.output_name][0][0] if score >= 0.1]
...
class MultipleOutputParser:
    ...
    def __call__(self, outputs):
        ...
        to_consider = [scores[:, 0] <= 0.9]
        sel_boxes = bboxes[to_consider]
        sel_scores = scores[to_consider][:,1]
        sel_labels = labels[to_consider]
        if len(sel_boxes) == 0:
            return []
        if len(sel_boxes) > 1:
            idxs = nms(sel_boxes[:,0], sel_boxes[:,1], sel_boxes[:,2], sel_boxes[:,3],sel_scores, 0.5)
            sel_boxes = sel_boxes[idxs]
            sel_scores = sel_scores[idxs]
            sel_labels  = sel_labels[idxs]
        # start = time.time()
        result = [Detection(*bbox, score, label)
                  for label, score, bbox in zip(sel_labels, sel_scores, sel_boxes)
                  if score < 0.9]
        # print(len(result), 1 / (time.time() - start))
        return result
