...
class AsyncPipeline:
    def __init__(self, ie, model, plugin_config, device='CPU', max_num_requests=1):
        ...
        self.logger.info('Network loaded with {} parallel executors'.format(len(self.exec_net.requests)))
        ...
        self.event.set()
        self.pendant = queue.Queue()
        self.working_thread = threading.Thread(target=self.do_work, daemon=True)
    ...
    def do_work(self):
        while True:
            self.event.wait()
            inputs, id, meta = self.pendant.get()
            result = self.do_submit_data(inputs, id, meta)
            if not result:
                self.pendant.put((inputs, id, meta))

    def inference_completion_callback(self, status, callback_args):
        try:
            request, callback, meta, preprocessing_meta = callback_args
            if status != 0:
            ...
            result = self.model.postprocess(raw_outputs, preprocessing_meta)
            callback(result, meta)
            # self.completed_request_results[id] = result
            self.empty_requests.append(request)

            self.processed_count += 1
            if time.time() - self.last_log_time > 5:
                logging.info(f"{self.processed_count} images processed by Network in 5s")
                self.last_log_time = time.time()
                self.processed_count = 0

        except Exception as Argument:
            logging.exception("Exception in inference_completion_callback.")
        self.event.set()

    def submit_data(self, inputs, callback, meta):
        self.pendant.put((inputs, callback, meta))

    def do_submit_data(self, inputs, id, meta):
        if len(self.empty_requests) == 0:
            logging.error("Empty request in async_pipeline. Something is wrong")
            self.event.clear()
            return False
